...
import {
  SlideNavigation,
  SLIDES,
} from "./components/SlideNavigation/SlideNavigation";
...
function App() {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      // Mapping: 0 = landing, 1 = comparison, 2 = context-window, 3 = lost-in-middle, 4 = visualizer, 5 = rpi-flow, 6 = error-cascade
      const slideMap = {
        0: "landing",
        1: "comparison",
        2: "context-window",
  ...
  const addElementToConversation = useCallback((convId, element) => {
    const instanceId = `${element.id}-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 9)}`;
    setConversations((prev) =>
      prev.map((conv) => {
        if (conv.id === convId) {
          return {
            ...conv,
            elements: [...conv.elements, { ...element, instanceId }],
          };
        }
        return conv;
      })
    ...
    setConversations((prev) =>
      prev.map((conv) => {
        if (conv.id === convId) {
          ...
          const existingSummaries = conv.elements.filter(
            (el) => el.id === "summary"
          );
          const elementsToSummarize = conv.elements.filter(
            (el) => !el.permanent && el.id !== "summary"
          );
          ...
          const totalTokens = elementsToSummarize.reduce(
            (sum, el) => sum + el.tokens,
            0
          );
          ...
          const summaryElement = {
            id: "summary",
            instanceId: `summary-${Date.now()}-${Math.random()
              .toString(36)
              .substr(2, 9)}`,
            name:
              summaryNumber > 1
                ? `Summary #${summaryNumber} (${elementsToSummarize.length} items)`
                : `Summary (${elementsToSummarize.length} items)`,
  ...
  const compactConversation = useCallback(
    (convId) => {
      ...
      const elementsToCompact = conv.elements.filter(
        (el) => !el.permanent && el.id !== "summary"
      );

      if (elementsToCompact.length < 1) return;

      const totalTokens = elementsToCompact.reduce(
        (sum, el) => sum + el.tokens,
        0
      );
      // More aggressive compression: 10-15% (vs 25-35% for /summarize)
      const compressionFactor = 0.1 + Math.random() * 0.05;
  ...
  const handleDragStart = (event) => {
    ...
    let element = Object.values(ELEMENT_TYPES).find(
      (el) => el.id === active.id
    );
    if (!element) {
  ...
  const conversationsWithSankey = useMemo(() => {
    return conversations.map((conv) => {
      const totalTokensUsed = conv.elements.reduce(
        (sum, el) => sum + el.tokens,
        0
      );
      const availableTokens = Math.max(
        0,
        CONTEXT_WINDOW_SIZE - totalTokensUsed
      );
  ...
  const isLanding = currentSlide === "landing";

  const handleGetStarted = () => {
    setCurrentSlide("comparison");
  };
  ...
  return (
    <div className="app">
      ...
      {currentSlide === "landing" && (
        <LandingSlide onGetStarted={handleGetStarted} />
      )}
